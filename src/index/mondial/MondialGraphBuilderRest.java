...
// 클래스 선언
public class MondialGraphBuilderRest extends GraphBuilder {
...
//	static final String field[] = {"name", "government", "name",  "name",   "name",   "name",    "name", "name", "name","abbreviation", "name","mountains","type", "name","islands","type", "name","type"};
	...
	public void createTest() {
		connect();
		String sql = "select id from auth_user limit 10";
		try {
	...
	public void createMemberRelation() {	
		...
		try {
			...
			while(rs.next()) {
				int oid = rs.getInt("oid"); 				
				int cid = rs.getInt("cid");
//				g.addEdge(oid, cid);	// country <- orga
				g.addEdge(cid, oid);	
//				edos.writeInt(oid);
//				edos.writeInt(cid);
				edos.writeInt(cid);
