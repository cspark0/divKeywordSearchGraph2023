...
//		double[] rels = new double[T.size()];
		...
		int len = T.size();
		ResultDestNodes = new int[len]; // ArrayList<Integer>(T.size());
		ResultRelevs = new double[len]; // ArrayList<Double>(T.size());
...
*/	}
	...
	private double computeAverageDissimilarity() {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < i; j++) {
				sumDiss += computeDissimilarityByDSC(
...
/*
		...
		pw.println();	
*/
		System.out.println("ResultDestNodes: " + Arrays.toString(ResultDestNodes));
...
//			srcNodes = n.getSrcNodes();
			...
			System.out.println("srcNodes: " + Arrays.toString(srcNodes));
