...
// - Maintaining Priority Q for each n and the states of previous exec of findUninqueAnswer()
//
...
public class BlinkSearcherBDB_NewIOOpt extends Searcher3 {

	HashMap<Integer, TopKNodeDataNewIOOpt> nodesTable = new HashMap<Integer, TopKNodeDataNewIOOpt>(32);
//	FibonacciHeap<Integer> T;				// global top-k queue T 
	NKMapRead nkmapRead;
	
	Set<Integer> visitedNodes = new HashSet<Integer>();
//	MinHeap Q = new MinHeap(1000);
//	FibonacciHeap<State> Q = new FibonacciHeap<State>(); 	 // priority queue for state space search
...
//	FibonacciHeapNode<State> node_s0;
	...
	public BlinkSearcherBDB_NewIOOpt() {
		super();
		initDictionary();	
	}

	public BlinkSearcherBDB_NewIOOpt(DirectedGraph<Integer, DefaultEdge> g) throws Exception {
		super(g);
		initDictionary();	
...
//      pool = new FibonacciHeapNodePool<State>(12500, spool);
	...
	public void completeSearch() {
		...
		Set<Entry<Integer, TopKNodeDataNewIOOpt>> eSet = nodesTable.entrySet();
		Iterator<Entry<Integer, TopKNodeDataNewIOOpt>> iter = eSet.iterator();
		while (iter.hasNext()) {		// iterate all top-k nodes in Qt
			Entry<Integer, TopKNodeDataNewIOOpt> e = iter.next();
	        FibonacciHeap<StateNew> stateQ = e.getValue().stateQ;
            Cursor[] curs = e.getValue().curs;
            if (stateQ != null) {       // then curs != null either
                stateQ.clear();
...
//		insertIntoTopKQueue(e.nodeID, srcNodes, score);
	    ...
	    FibonacciHeap<StateNew> stateQ = null;
        Cursor[] curs = null;
        RelSource[][] P = null;
    	TopKNodeDataNewIOOpt nodeData = null, dupNodeData = null;
        NodeScore ns = null;
        ...
        if (dupId == 0) {   // no duplication found!
            ...
            insertIntoTopKQueue(new TopKNodeDataNewIOOpt(srcNodes, sortedNodes, ns, stateQ, sol, P, curs));
            return;
        }
...
//            System.out.println("A duplicate answer found, rooted at " + curId);
            ...
            // first try: sol == null
            stateQ = new FibonacciHeap<StateNew>(); 
            P = new RelSource[numOfQueryKeywords][];
            curs = new Cursor[numOfQueryKeywords];

            start = cpuutil.getCpuTime();
            sol = findUniqueAnswer(curId, P, curs, /*curScore,*/ null, stateQ, query, srcNodes, sortedNodes);
...
//            System.out.println("A duplicate answer found, rooted at " + dupId);
        ...
        while (true) {
            ...
            if (dupNodeData.ns.score <= min_k) {
                if (dupNodeData.stateQ != null) {     
                    dupNodeData.stateQ.clear();
                    for (i = 0; i < numOfQueryKeywords; i++) 
            ...
            if (dupNodeData.solState == null) {        // first try
                dupNodeData.stateQ = new FibonacciHeap<StateNew>(); 
		        dupNodeData.P = new RelSource[numOfQueryKeywords][];
                dupNodeData.curs = new Cursor[numOfQueryKeywords];
            }

            start = cpuutil.getCpuTime();
    		sol = findUniqueAnswer(dupId, dupNodeData.P, dupNodeData.curs, /*ns.score,*/ dupNodeData.solState, dupNodeData.stateQ, query, srcNodes, sortedNodes);
           end = cpuutil.getCpuTime();
            accFindUniqTime += (end - start);
...
//            System.out.println("A duplicate answer found, rooted at " + gNextDupId);
	...
	public void insertIntoTopKQueue(TopKNodeDataNewIOOpt nodeData) {
...
//				System.out.print("T -> (node " + TNode.getData() + ", " + TNode.getKey() + ") -> ");
                ...
                TopKNodeDataNewIOOpt n = nodesTable.remove(removed.nodeId);
...
//		int[] theSortedNodes = sortSrcNodes(srcNodes);	// sort srcNodesIDs
	...
	public StateNew findUniqueAnswer(int destNode, RelSource[][] P, Cursor[] curs, /*float prevScore,*/ StateNew prevSol, FibonacciHeap<StateNew> Q, List<String> query, int[] srcNodes, int[] sortedSrcNodes) {  // srcNodes and sortedSrcNodes will be returned 
		int i, j, k, dupId;
...
//            lb_s = score_s + sum_worst[0];
        ...
        else {      // prevSol != null: not the first try
            ...
            // here, prevSol becomes a duplicate solution!
...
//                        lb_s = score_s + sum_worst[i];
  //                      if (lb_s <= UB) {				
                            ...
                            if (dupId != destNode) {    // this is a new best unique solution state!
                                try {
                                        if (bestSol != null) // store into Q for later use
                                        {  bestSol.solChecked = false;
                                           Q.insert(pool.getFibonacciHeapNode(bestSol), -bestSol.ub);
                                        }
                                   bestSol = spool.getState(i, e.indJ, score_s, ub_s, true);
                                   bestSol.indJ[i]++;
                                } catch (Exception ex) { ex.printStackTrace(); }
                                LB = ub_s;
                                ...
                                try {
                                   // solChecked = true;
                                   s = spool.getState(i, e.indJ, score_s, ub_s, true);
                                   s.indJ[i]++;
                                   Q.insert(pool.getFibonacciHeapNode(s), -ub_s);
                                } catch (Exception ex) { ex.printStackTrace(); }
                            }
//                        }
                        ...
                        try {
                           ...
                           s = spool.getState(i, e.indJ, score_s, ub_s, false);
                           s.indJ[i]++;
                           Q.insert(pool.getFibonacciHeapNode(s), -ub_s);
                        } catch (Exception ex) { ex.printStackTrace(); }
                        }
...
//              if (lb_s <= UB) {				
//                    try {   // solChecked = true!
//                        s = spool.getState(i, 0, score_s, ub_s, true);
//                      Q.insert(pool.getFibonacciHeapNode(s), -ub_s);
//                    } catch (Exception ex) { ex.printStackTrace(); }
 //             }
...
//              e = s;
		...
		while (!Q.isEmpty()) {
			FibonacciHeapNode<StateNew> n = Q.min();
			e = n.getData();
//			e = Q.poll();
			if (e.ub <= LB) { 
//    			pool.returnFibonacciHeapNode(n);
 //               spool.returnState(e);
            ...
            // here e.ub > LB
            if (e.solChecked == false) {    // e is not checked yet!
                 
                // check duplication of srcNodes this state represents
                    k = numOfQueryKeywords - 1;
                    for (; k > e.i; k--) 
                        sortedSrcNodes[k] = P[k][0].srcNode;
                    for (; k >= 0; k--)    // k == e.i
                    ...
                    // score_e = e.ub
                    dupId = checkDuplicateAnswer(destNode, sortedSrcNodes, e.ub);
                    if (dupId != destNode) {    // e is a new best unique solution state! --> no need to process its subtree any further!
                        if (bestSol != null) // store into Q for later use
                        {  bestSol.solChecked = false;
                            try {
                                Q.insert(pool.getFibonacciHeapNode(bestSol), -bestSol.ub);
...
//		return destNodes;
	...
	public int countNumberOfDuplicateAnswers(int[] nodes, Map<Integer, TopKNodeDataNewIOOpt> nodesTable) {
		for (int u : nodes) {
			srcNodesSet.add(
	...
	public int countNumberOfUniqueAnswers(List<Integer> nodes, Map<Integer, TopKNodeDataNewIOOpt> nodesTable) {
		for (int u : nodes) {
			srcNodesSet.add(
                Arrays.hashCode(nodesTable.get(u).sortedSrcNodes)
            );
