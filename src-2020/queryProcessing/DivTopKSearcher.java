...
public class DivTopKSearcher extends Searcher {
	...
	List<AnswerTree> optimalAnswer;		// optimal div top-k answer!
	...
	private void init() {
		R = new PriorityQueue<AnswerTree>(32, new AnswerTreeComparator()); 			// worstScore의 내림차순으로 최대 k개 저장; root node == min-k node
...
//			System.out.println("Next top tree: " + R.peek());
	...
	protected float checkDissCondition(List<AnswerTree> S, AnswerTree t, double sumDissOfS) {
		// return new SumDiss value Of S Union {t} if diss condition is satisfied, 
		//                                         else -1
		for (int i = 0; i < S.size(); i++) {
			sumDissOfS += computeDissimilarity(S.get(i).srcNodes, t.srcNodes);
		}
	...
	protected void getResult(List<AnswerTree> answer, float sumDiss) { // List<AnswerTree> ans) {	
