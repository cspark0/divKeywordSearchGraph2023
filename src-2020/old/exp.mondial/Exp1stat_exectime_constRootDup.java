...
public class Exp1stat_exectime_constRootDup {
...
//		BlinkSearcherBDB_Reduced2 searcher = new BlinkSearcherBDB_Reduced2(g);
//		searcher[3] = new BlinkSearcherBDB_ReducedUnique(g);
		BlinkSearcherBDB_ConstRootDup searcher = new BlinkSearcherBDB_ConstRootDup(g);
...
//		String[] M = {"blinks", "blinks-NonRed", "nonRed", "nonRednonDup" };
		...
		f2 = new File(Params.ExpDB + "/rootdup/" + "constRootDup" + "-stat-exectime.txt" + args[0]);
		try {
...
//				for (int i = 0; i < K.length; i++) {		
				Params.K = Integer.parseInt(args[0]);
//				pw1.println("klafkjjjkklink Search Using BDB " + K[i]);
