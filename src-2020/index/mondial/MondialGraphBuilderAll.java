...
// 클래스 선언
public class MondialGraphBuilderAll extends GraphBuilder {
...
//	static final String field[] = {"name", "government", "name",  "name",   "name",   "name",    "name", "name", "name","abbreviation", "name","mountains","type", "name","islands","type", "name","type"};
	...
	public void createCountry() {
		...
		try {
			...
			while(rs.next()) {
				...
				edos.writeInt(cid); edos.writeInt(contid);
				g.addEdge(contid, cid);		
				edos.writeInt(contid); edos.writeInt(cid);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void createPolitics() {
		...
		try {
			...
			while(rs.next()) {
				...
				g.addEdge(pid, crid);
				edos.writeInt(pid); edos.writeInt(crid);				
				g.addEdge(crid, pid);		
				edos.writeInt(crid); edos.writeInt(pid);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void createReligion() {
		...
		try {
			...
			while(rs.next()) {
				...
				g.addEdge(rid, crid);
				edos.writeInt(rid); edos.writeInt(crid); 
				g.addEdge(crid, rid);	
				edos.writeInt(crid); edos.writeInt(rid);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void createCity() {
		...
		try {
			...
			while(rs.next()) {
				...
				g.addEdge(ctid, pid);
				g.addEdge(pid, ctid);	
				edos.writeInt(ctid);edos.writeInt(pid);
				edos.writeInt(pid); edos.writeInt(ctid);
			}
			rs.close();	pstmt.close();
			...
			while(rs.next()) {
				...
				g.addEdge(ctid, crid);
				g.addEdge(crid, ctid);
				edos.writeInt(ctid); edos.writeInt(crid);
				edos.writeInt(crid); edos.writeInt(ctid);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void createOrganization() {
		...
		try {
			...
			while(rs.next()) {
				...
				edos.writeInt(oid);	edos.writeInt(cid);
				edos.writeInt(cid);	edos.writeInt(oid);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void createRiver() {
		...
		String sql5 = "select l.id as lid, r.id as rid " +		// lake - river
				"from lake0 l, river0 r where l.river = r.name";
		String sql6 = "select r.id as rid, s.id as sid " +		// river - sea
